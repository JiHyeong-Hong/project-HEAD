...
public class Enemy : LivingEntity
{
    ...
    private void OnTriggerStay(Collider other)
    {
        ...
        if(!dead && Time.time >= lastAttackTime + timeBetAttack)
        {
            ...
            LivingEntity attackTargt = other.GetComponent<LivingEntity>();

            // 상대방의 LivingEntity가 자신의 추적 대상이면 공격 실행
            if(attackTargt != null && attackTargt == targetEntity)
            {
                // 최근 공격 시간 갱신
                ...
                // 공격 실행
                attackTargt.OnDamage(damage, hitPoint, hitNormal);
            }
        }
    }
